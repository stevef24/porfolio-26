...
You send a request. The runtime forwards it to the model along with the tools it can use. The model looks at the request and decides what to do next. If it wants to use a tool, it signals that. The runtime then executes that tool, captures the result, and sends it back to the model. The model looks at the new information and decides what to do next. This keeps going until the model signals that it is done. So instead of one request and one response, you now have a loop where reasoning and execution take turns.
...
If you're building agents, you're not just working with an LLM. You're designing a system around it. The harness gives the model hands. The runtime decides how those hands are used.

The agent loop is not as simple as "an LLM in a loop." There is a lot of thinking and engineering around it, and that is what makes it so interesting. This article covered the basics. In the next ones, I'll go deeper into things like tool design, MCP integration, prompt structure, and context management. But having this foundational understanding is what everything else builds on.
